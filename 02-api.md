...
3. 해당 엔드포인트는 다음과 같은 형태의 응답을 가진다.
    ...
    - 제공되는 데이터는 잘 정렬되어 있어야 하며, 그 개수는 `limit`보다 **작거나 같아야** 한다.
...
#### 오류 응답
- 이 홈서버에 등록되지 않은 아이덴티티라면, **404 Not Found** (`E_NOTFOUND`)
...
- 증표의 정렬 방법은 서버가 정하지만, 전체 목록은 잘 정렬되어 있어야 한다.
...
페이지네이션은 에라타 생성 시간 내림차순으로 정렬된 목록에 대해 동작해야 한다. 달리 표현하면, `offset=0`일 때의 응답은 항상 가장 최신 에라타를 보여줄 수 있어야 한다.
