...
5. `data.intent`는 정의된 것과 동일한 값이어야 한다.

또한 서버는 `preferredHandle` 값이 있을 경우, 그 핸들네임이 실제로 해당 아이덴티티를 가리키는지 확인해야 한다.
- 핸들이 같은 서버에서 발급한 핸들이라면, 즉시 확인할 수 있을 것이다.
- 핸들이 다른 서버에서 발급한 핸들이라면, GET `https://<DOMAIN>/_deps/home/handles/[handle]` 요청을 통해 확인할 수 있다.
...
- `preferredHandle`이 실제로 해당 아이덴티티를 가리키지 않는다면, **400 Bad Request** (`E_INVALID_REQUEST`)
...
### POST `/identities/[identity]/solves`
...
- `data`: 데이터.
    ...
    - `intent`: `deps/postIdentitySolveCert`로 고정된 문자열.
- `sign`: **아이덴티티의** 서명.
...
서버는 받은 요청에 대해 아래와 같이 검증을 수행해야 한다.
1. `key`가 `[identity]`와 같음을 확인한다.
2. 서명 검증을 수행한다.
    - `sign`이 올바른 서명인지 확인한다.
    - `data.intent`가 `deps/postIdentitySolveCert`와 같음을 확인한다.
    - `data.signedAt`가 허용 범위 (e.g. 60초) 내에 있음을 검증한다.
3. **문제증표의 검증**을 수행한다.
4. 아래와 같이 중복 확인을 수행한다.

서버는 동등한 문제증표를 이미 가지고 있다면, 그 중 `signedAt` 값이 더 최신인 것만 남긴다. 동등한 문제증표의 정의는 다음과 같다.
- `data.identity.key`가 동일하다.
- `data.problemId`가 동일하다.
- `data.score`가 동일하다.

서버는 `data.identity.key`와 `data.problemId`가 동일하지만 `data.score`가 다른 문제증표가 있다면, 둘을 동시에 남길 수도 있고 `signedAt`와 관계없이 `data.score`가 더 높은 쪽만 남길 수도 있다. 이 경우의 처리 방법은 서버 구현체가 적절하게 결정한다.

#### 오류 응답
- 이 홈서버에 등록되지 않은 아이덴티티라면, **404 Not Found** (`E_NOT_FOUND`)
- `key`가 `[identity]`와 다르다면, **400 Bad Request** (`E_INVALID_REQUEST`)
- 서명 검증에 실패했다면, **403 Forbidden** (`E_INVALID_SIGN`)
- `signedAt`가 허용 범위를 벗어났다면, **403 Forbidden** (`E_OUTDATED_SIGN`)
...
- 그 외의 사유로 문제증표가 거부되었다면, **400 Bad Request** (`E_INVALID_REQUEST`)
...
- `data`: 제출 데이터
    ...
    - `signedAt`: 제출 시각
    - `intent`: `deps/problemSubmit`로 고정된 문자열.
- `sign`: data에 대한 서명
...
2. 서명 검증을 수행한다.
    - `sign`이 올바른 서명인지 확인한다.
    - `signedAt` 값이 허용 범위 (e.g. 180초) 내에 있는지 확인한다.
    - `intent` 값이 `deps/problemSubmit`와 같은지 확인한다.
...
**이전 문서** [01 - 데이터 정의](01-definitions.md)
**다음 문서** [03 - 프로토콜 익스텐션](03-extensions.md)
