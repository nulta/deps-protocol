...
1. (필요시) `data.identity`가 원하는 아이덴티티의 식별자가 맞는지 대조한다.
2. `data.problemId`로부터 문제서버의 도메인을 받아온다.
3. 그 문제서버의 공개키를 얻어온다.
    - 문제서버 공개키를 이미 알고 있다면, 건너뛸 수 있다.
    - 아니라면, GET `https://<DOMAIN>/_deps/judge/info` 요청을 통해 공개키를 얻어온다.
4. `key`가 문제서버의 공개키가 맞는지 확인한다.
5. `sign`을 문제서버 공개키로 검증한다.
6. 문제서버의 에라타를 받아온다.
...
7. 해당 문제에 에라타가 없거나, 선언 시점이 `data.signedAt`보다 과거가 맞음을 확인한다.
